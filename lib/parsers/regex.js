 ...
 * @params {array} fields
 * @params {bool} nowarn suppress warning if no regex matches
 */
...
function RegexParser(regex, fields, nowarn) {
...
RegexParser.prototype._transform = function(buf, enc, cb) {
  ...
  if(!this.nowarn) api.logger.warn('No Regex did match: '+buf.toString());
  return cb();
};


api.parsers.regex = function(config) {
  return new RegexParser(config.regex, config.fields, config.nowarn);
};