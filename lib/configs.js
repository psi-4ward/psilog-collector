...
function loadConfig(cb) {
  async.each(api.argv.configdir, function(confdir, cb) {
    fs.readFile(confdir + '/main.yml', {encoding: 'utf8'}, function(err, data) {
      if(err) return cb();
      api.logger.debug('Loading config: ' + api.argv.configdir + '/main.yml');
      try {
        if(!api.config) api.config = {};
        _.merge(api.config, yaml.safeLoad(data));
        cb();
      }
      catch(e) {
    ...
    if(api.config.log && typeof api.config.log.colorize !== undefined) {
      api.logger.transports.console.colorize = api.config.log.colorize;
    }
    if(api.config.log && api.config.log.timestamp === false) {
      api.logger.transports.console.timestamp = null;
    }
