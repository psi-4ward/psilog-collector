 ...
 * Convert a Date string into an unix timestamp
...
function ParseDate(field, target, format) {
...
ParseDate.prototype._transform = function(obj, enc, cb) {
  ...
  if(obj[this.field]) {
    ...
    if(d.isValid()) {
      obj[this.target] = d.format('X');
    } else {
      api.logger.warn('Could not parse Date:' + obj[this.field]);
    }
  }
  this.push(obj);
  cb();
...
api.transformers.parseDate = function(config) {
  return new ParseDate(config.field || '@timestamp', config.target || '@timestamp', config.format);
};