...
var deltaCache = {};
var container_stats_variant = 'v1';
...
api.inputs.docker = function(config, cb) {
  ...
  Modem.dial({
    ...
    if(err) {
      return;
      api.logger.error('Docker', err.toString());
    }
    if(parseFloat(json.ApiVersion) >= 1.19) container_stats_variant = 'v2';
  });
...
var container_stats = {
  v1: function container_stats_v1(cid, cb) {
  Modem.dial({
  ...
  v2: function container_stats_v2(cid, cb) {
    Modem.dial({
      path: '/containers/' + cid + '/stats?stream=0',
      method: 'GET',
      statusCodes: {
        200: true,
        404: "no such container",
...
function pushData(readstream, config) {
  ...
  Modem.dial({
    ...
    async.eachLimit(runningContainers, 10, function(container, next) {
      ...
      container_stats[container_stats_variant](container.Id, function(err, stats) {
        if(err) {
          api.logger.error('Docker', err.toString());
          return next();
        }
